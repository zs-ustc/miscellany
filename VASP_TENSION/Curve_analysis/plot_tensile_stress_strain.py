#!/usr/bin/python   
#This code is for data analysis and plotting. 
### ================================= read me first =================================
# Qing Peng @ Aug 05, 2021
# This python code is for the data analysis of a tensile simulation. 
# The resulting data should be in a file (for example, stress_strain) 
# It can be generated using the following LAMMPS syntex 
#  fix  ss  all ave/time 1 100 250 v_strain v_stress file stress_strain

# ---------------  plot_tensile_stress_strain.py -----------------------------------
...
#    5. determine the failure strain
# 
...
def usage(msg=''):
    ss=''' Usage example: python3 ./plot_tensile_stress_strain.py result_CNT_tensile.dat --title 'Defective CNT' --xlim 0 0.2 --ylim 0 60 --Yn 50
    '''
    ss += msg
    print(ss)
    return ss
...
parser.add_argument('inputDataFile', help='input Data File',default='stress_strain')
parser.add_argument('-t',help='fitting-type',default='')
parser.add_argument('--file2', help='inputfile2',default='')
parser.add_argument('--title', help='title of the figure',default='Tensile')
parser.add_argument('--j1', help='x colomn',type=int,default=1)
parser.add_argument('--j2', help='y colomn',type=int,default=2)
...
parser.add_argument('--label', help='label',default='DFT:fitting')
parser.add_argument('--lw', help='linewidth',type=float,default=1.5)
...
parser.add_argument('--xfsize', help='axis fontsize',type=int,default=12)
parser.add_argument('--txtfsize', help='text size',type=int,default=9)
parser.add_argument('--legfsize', help='legend text size',type=int,default=9)
parser.add_argument('--loc', help='legend location',default='lower right')
parser.add_argument('--dpi', help='resolution',type=float,default=300.0)
parser.add_argument('--I2D', help='Dimension',type=int,default=0)
...
color_fitting = (6/255,121/255,159/255)
...
Data=np.loadtxt(fname,skiprows=1)                              #import the data in the text with specified path
strain=Data[:,args.j1-1]/100                            #define the strain values # the second column
...
x1 =np.linspace(0,0.2,100)                          #define an array with 100 points from 0 to 0.1
if args.file2 != '':
    M=np.loadtxt(args.file2) 
    X2,Y2=M[:,args.j1-1],M[:,args.j2-1]
    x2,y2=X2[:n],Y2[:n]

if args.t.lower() in ['1','linear']:                                       # fitting type 1: using polyfit 
...
elif args.t.lower() in ['y','young','youngs','proportional','prop']:
...
if args.I2D == 1: xlab = 'Strain:Stress (N/m)'.split(':')  
l1,=plt.plot(strain, stress, linestyle = '-',  color = 'b', label=lab[0], linewidth=args.lw)  #plot the stress-strain curve
plt.scatter(strain, stress, marker='+', color = 'coral')
# add the fitting curve
if args.file2 != '':
    if args.t: l2,=plt.plot(x1,yfit,label=lab[1],linewidth=args.lw2,color="r",linestyle="--",alpha=args.alpha) #Plot the fit data
else:
    if args.t: l2,=plt.plot(x1,yfit,label=lab[1],linewidth=args.lw2,color=color_fitting,linestyle="--",alpha=args.alpha) #Plot the fit data
...
if args.file2 != '':
    ...
    if args.t: l22,=plt.plot(x1,yfit2,'b:',label=lab[3], linewidth=args.lw2,alpha=args.alpha)    #plot the stress-strain curve
...
if args.t :
    ...
    if args.I2D == 1 :
        ss = "   Data analysis \n"
        ss +="Young's  modulus: %.1f N/m \n"%Young
        ss +="Material strength:  %.1f N/m \n"%Strength
        ss +="Material toughness: %.3f \n"%Toughness
        ss +="Fracture strain:    %.3f \n"%Fstrain
    if args.file2 != '':
        Young2=k2
...
if args.t: #                                  saving fitting results
    print(ss)
    fw=open(args.title+'_fitting_results.txt','w')
    fw.write(ss)
    fw.close()
